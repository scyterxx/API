...
async fn run_service(options: &Options) -> Result<(), anyhow::Error> {
    ...
    // Save contexts for flushing during shutdown
    let module_contexts_for_tasks = module_contexts.clone(); 
    
    // Start internal loops for all modules via MonitorManager
    let mut tasks = monitor_manager
        .start_modules(module_contexts_for_tasks, shutdown_notify.clone())
        .await?;
    ...
    // Dapatkan handles untuk penanganan sinyal (SIGINT/SIGTERM/SIGHUP)
    // NOTE: ctrl_c() returns a future, which we poll directly. Unix signals need .recv().
    let sigint_future = signal::ctrl_c();
    ...
    tokio::select! {
        _ = sigint_future => {
            info!("Menerima SIGINT (Ctrl+C). Memulai shutdown...");
        }
        _ = sigterm.recv() => {
            info!("Menerima SIGTERM. Memulai shutdown...");
        }
        _ = sighup.recv() => {
            info!("Menerima SIGHUP. Memulai shutdown...");
        }
    }

    // 1. NOTIFY WAITERS: Beri tahu semua loop modul untuk berhenti.
    info!("Memberi tahu modul untuk berhenti...");
    shutdown_notify.notify_waiters();

    // 2. TUNGGU SEMUA MODUL SELESAI (JOIN TASKS): Ini memastikan writer/loop berhenti.
    info!("Menunggu task modul selesai...");
    for handle in tasks {
        if let Err(e) = handle.await {
            log::error!("Error saat bergabung dengan task modul: {}", e);
        }
    }

    // 3. FLUSH RING DATA: Lakukan flush/fsync setelah semua task writer selesai.
    info!("Semua modul berhenti, melakukan FLUSH RING DATA ke disk...");
    // Pass the original contexts for flushing
    monitor_manager.flush_all_modules(&module_contexts).await; 

    info!("Flush selesai. Proses keluar dengan sukses.");
...
pub async fn run(options: Options) -> Result<(), anyhow::Error> {
    ...
    let log_level = match options.log_level().to_lowercase().as_str() {
        ...
        _ => {
            return Err(anyhow::anyhow!( // FIX: changed anys to anyhow!
