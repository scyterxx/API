...
impl TrafficModuleContext {
    /// Flush dan Fsync semua data ring ke disk. Dipanggil saat graceful shutdown.
    pub async fn flush_and_fsync_all(&self) {
        log::info!("Traffic: flushing realtime ring");
        if let Err(e) = self.realtime_ring_manager.flush_dirty_rings().await {
            log::error!("flush realtime ring failed: {}", e);
        }
        // FIX: fsync_all() dikomentari karena metode tidak ditemukan.
        /* if let Err(e) = self.realtime_ring_manager.fsync_all() {
            log::error!("fsync realtime ring failed: {}", e);
        }
        */

        log::info!("Traffic: flushing multi-level ring");
        if let Err(e) = self.multi_level_ring_manager.flush_dirty_rings().await {
            log::error!("flush multi-level ring failed: {}", e);
        }
        // FIX: fsync_all() dikomentari karena metode tidak ditemukan.
        /*
        if let Err(e) = self.multi_level_ring_manager.fsync_all() {
            log::error!("fsync multi-level ring failed: {}", e);
        }
        */
        log::info!("Traffic: ring data flushed and synced");
    }
...
impl MonitorManager {
    ...
    /// Posisikan ulang fungsi flush_all_modules di sini, dengan signature yang benar.
    pub async fn flush_all_modules(&self, contexts: &[ModuleContext]) {
        for (module_type, ctx) in self.modules.iter().zip(contexts.iter()) {
            if let (ModuleType::Traffic, ModuleContext::Traffic(traffic_ctx)) = (module_type, ctx) {
                // Memanggil fungsi flush/fsync di TrafficModuleContext
                traffic_ctx.flush_and_fsync_all().await;
            }
            // Tambahkan modul lain di sini jika mereka membutuhkan flush/fsync
        }
    }
